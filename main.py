...
@isLogin
def main():
    data = db_api.get_all_hero()
    category = db_api.get_all_category()
    user = db_api.get_user(session["user_id"])
    return render_template("main.html", data = data, category = category, user = user)

@app.route("/main/category/<id>", 
           endpoint = "category", 
           methods = ["POST","GET"])# <a href="localhost:5000/hero/{{id}}">кнопка</a>
@isLogin
def category(id:int):
    data = db_api.get_form_category_hero(id)
    category = db_api.get_all_category()
    user = db_api.get_user(session["user_id"])
...
@isLogin
def hero(id:int):
    data = db_api.get_hero(id)
    all_spell = db_api.get_spell(id)
    user = db_api.get_user(session["user_id"])
